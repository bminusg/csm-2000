...
class LocalConnection {
  ...
  reconnect() {
    ...
    for (n = 0; n < self.frames.length; n++) {
      self[self.frames[n]] =
        self[self.frames[n]] || self.getFrameByName(self.frames[n]);
    }
  ...
  getFrameByName(frame) {
    ...
    while (i < top.frames.length) {
      ...
      try {
        checkedFrame =
          typeof childFrame.LC === "undefined"
            ? null
            : self.checkFrame(childFrame, frame);
      } catch {}
  ...
  checkFrame(checkThisOut, frame) {
    ...
    try {
      if (
        checkThisOut.LC.key === self.key &&
        checkThisOut.LC.name === frame &&
        checkThisOut.LC.ready
      ) {
        /**
